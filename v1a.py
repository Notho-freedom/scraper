...
MAX_PAGES = 50
CONCURRENT_TASKS = 10
SAVE_FILE = "reaper_index_textblob.json"
...
def extract_text(html):
    ...
    for s in sentences:
        ...
        errors = 0 if corrected == s else 1
...
async def crawl(url, session, depth=0, sem=None, pbar=None):
    ...
    async with sem:
        ...
        corrected_sentences = extract_text(html)
        word_count = sum(len(s["corrected"].split()) for s in corrected_sentences)
...
async def main():
    ...
    print(Fore.GREEN + "\n===== RAPPORT ULTRACORE MOBILE =====" + Style.RESET_ALL)
